\section{实验结果}
为了验证 EditFlow 在符号回归任务中的有效性，我们在 Feynman Symbolic Regression Benchmark 数据集上进行了评估。需要注意的是，近期基准研究指出 Feynman 系列任务的采样区间设置与“纯数值误差”指标可能不足以刻画结构可解释性与科学发现需求，因此建议引入结构相似度度量（如归一化编辑距离）并关注更严格的泛化协议\cite{matsubara2024rethinking}；面向大模型的 LLM-SRBench 亦强调应控制记忆效应、提升对推理与外推能力的考察\cite{shojaee2025llmsrbench}。基于上述观点，本文的编辑式后处理框架可被视为对“结构纠错/结构接近度”这一评测关注点的直接响应。本节聚焦两类关键场景：(1) 对受损物理方程的结构修复能力；(2) 作为后处理模块与遗传编程方法（PySR\cite{cranmer2023pysr}）的协同优化能力。
...
遗传编程（如 PySR）在符号回归中表现优异，但在高维或复杂函数上常受限于搜索时间，容易陷入局部最优。为此，我们构建“PySR + EditFlow”的混合策略：首先运行 PySR 进行短时搜索（迭代预算为标准设置的 10\%），再将其得到的次优解输入 EditFlow 进行精炼。
