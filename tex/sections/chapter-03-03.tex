...
\noindent 图\ref{fig-3-3-5-generalization-error}把泛化问题集中呈现为"模型复杂度与误差的关系"。上排三幅图分别示意欠拟合、良好泛化与过拟合：欠拟合时模型过于简单，连训练集的主要趋势都抓不住，训练误差与测试误差都偏高；良好泛化时模型复杂度适中，既能贴合训练数据的主要规律，又能在未见数据上保持较小误差；过拟合时模型过于复杂，把训练数据中的噪声也当成规律记住，训练误差极低但测试误差显著升高。下方曲线进一步抽象出这一现象：随着模型复杂度从低到高，训练误差通常单调下降，而测试（泛化）误差往往呈先降后升的"U"形，中间阴影区域对应最佳泛化区，即在偏差与方差之间取得更合理的折中。该图所强调的结论是：泛化并不等价于把训练误差压到最低，而是要在模型表达能力与稳定性之间做权衡，使测试误差处在其最低附近的区间。
