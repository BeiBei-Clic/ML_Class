...
\noindent 从图中可以看出，训练阶段模型同时获得输入$X$与输出$y$，通过学习两者之间的映射关系来调整参数；而部署阶段模型只接收新的输入$X$，输出预测值$\hat{y}$。这种区分确保了模型不会依赖预测时无法获得的信息。
...
\noindent 图中展示了这四个概念的层次关系。模型定义了函数的形式，假设空间是所有可能的候选函数集合，参数是具体实例化一个函数的自由度，而超参数则控制假设空间的规模和训练过程。理解这一关系有助于后续对训练和优化过程的理解。
...
\noindent 图中展示了训练过程的三个核心要素。损失函数把单个样本的预测误差量化成数值，目标函数是所有样本损失的平均可能加上正则项，优化则是通过调整参数来最小化目标函数的过程。这三者共同构成了训练的数学框架。
...
\noindent 从图中可以清晰看出三个数据集的职责边界。训练集负责学习参数，验证集负责模型选择和调参，测试集只负责最终一次性评估。三者不能混用，否则评估结果将失去参考价值。
...
\noindent 图中展示了误差的三个主要来源。不可约误差来自测量噪声和信息不足，是任务本身的上限。系统性误差来自模型表达能力不足或特征缺失，表现为稳定的偏差。不稳定误差来自模型对训练数据扰动过于敏感，表现为预测波动。理解这三类误差有助于制定针对性的改进策略。
...
\noindent 从图中可以看出监督学习的三个核心环节。首先用带标签的数据训练模型，训练过程通过优化目标函数来调整参数。然后在验证集上评估并选择最好的模型。最后在测试集上给出最终性能报告。这一流程确保模型不仅训练得好，而且在新数据上也能保持稳定表现。
...
\noindent 与监督学习相比，无监督学习最关键的差异在于训练信号来自"数据自身"。因此，问题的定义往往更依赖建模假设：你需要明确你认为数据里应该存在什么样的结构，或你希望保留什么样的信息。换句话说，无监督学习通常不是在问"预测对不对"，而是在问"用某种结构或表示去解释数据是否合理"。无监督学习的基本流程如图\ref{fig:3-3-3-unsupervised-learning}所示。
...
\noindent 图中展示了无监督学习的典型流程。与监督学习不同，无监督学习只有输入数据，没有标签。算法从数据自身发现结构，例如聚类、降维或表示学习。评估标准通常来自数据内部的紧密度或一致性，而不是外部的标签匹配。
...
\noindent 下面用Python代码实现K-means聚类的核心迭代过程。该代码与手算实例完全对应，其中数据为一维数组 $[1,2,4,8]$，聚类数设为 $K=2$。
...
其中 $\gamma\in[0,1)$ 是折扣因子，用来控制未来奖励相对于当前奖励的重要程度。强化学习的目标可以概括为：学习一条策略，使得期望回报 $\mathbb{E}[G_0]$ 最大。强化学习的基本交互结构如图\ref{fig:3-3-4-reinforcement-learning}所示。
...
\noindent 从图中可以看出强化学习的核心交互循环。智能体观察状态并选择动作，环境根据动作给出奖励并转移到新状态。与监督学习不同，强化学习的训练信号不是标准答案，而是对行为结果的评价。智能体的目标是通过反复交互，学习一套使长期回报最大化的策略。
