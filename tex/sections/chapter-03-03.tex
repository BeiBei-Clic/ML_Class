...
\noindent \textbf{（对应图\ref{fig:3-3-1-input-output}的段落替换稿）} 从图\ref{fig:3-3-1-input-output}可以更直观地把"数据如何组织成可学习对象"说清楚。输入数据被写成特征矩阵 $X\in\mathbb{R}^{n\times d}$：第 $i$ 行对应第 $i$ 个样本（sample），即该样本的特征向量 $\boldsymbol{x}_i^\top=(x_{i1},\dots,x_{id})$；第 $j$ 列对应第 $j$ 个特征（feature），表示这一属性在所有样本上的取值。输出数据写成标签向量 $\boldsymbol{y}\in\mathbb{R}^n$（或二分类时 $\boldsymbol{y}\in\{0,1\}^n$），其中 $y_i$ 是与样本 $\boldsymbol{x}_i$ 配对的监督信号。学习的本质是用模型 $f(\cdot;\theta)$ 把每个样本的特征向量映射为预测 $\hat{y}_i=f(\boldsymbol{x}_i;\theta)$，并通过比较 $\hat{y}_i$ 与 $y_i$ 的差异来调整参数 $\theta$。因此，图中"行=样本、列=特征、$(\boldsymbol{x}_i,y_i)$ 成对输入训练"的表示方式，明确了机器学习计算所依赖的最基本数据结构。
...
其中 $y_i$ 是第 $i$ 套房子的成交价格。写成这种形式，并不是为了符号整齐，而是为了明确训练与部署阶段的信息边界：训练阶段模型观察配对样本 $(\boldsymbol{x}_i,y_i)$ 来学习从输入到输出的映射规律；部署阶段面对的是一套新房子对应的新向量 $\boldsymbol{x}_{\text{new}}$，模型只能使用这些在预测时可得的分量输出预测值 $\hat y_{\text{new}}$。仍以房价为例，每套房子是一条样本，面积、卧室数、楼龄、是否地铁房分别对应 $X$ 的四列。将数据组织为矩阵并非为了形式美观，而是为了把对象统一成可计算的表示，从而使学习一个从 $X$ 到 $\boldsymbol{y}$ 的映射成为明确的数学问题。
