...
\noindent 在形式化层面，最常用的表达是区分输入与输出。通常用 $X$ 表示输入数据，常见形式是特征矩阵或设计矩阵，用 $y$ 表示输出数据，也就是目标、标签或响应。$X$ 表示预测时可获得的描述信息，$y$ 表示训练阶段用作监督信号而在预测时不可直接使用的目标信息。以房价预测为例，$X$ 可以包含面积、户型、地段、楼龄、周边配套等属性，$y$ 是成交价格。训练阶段模型观察 $(X,y)$ 来学习映射规律；部署阶段模型仅接收新的 $X$，输出预测值 $\hat{y}$。这一区分看似只是符号约定，实质上规定了信息可得性的硬约束：任何预测时无法获得的信息，都不应被纳入 $X$。如果把未来信息或与目标几乎同义的字段混入特征，离线指标往往会异常乐观，而上线表现会迅速变差。这一关系如图\ref{fig-3-3-1-input-output}所示。
...
\noindent 从图\ref{fig-3-3-1-input-output}可以更直观地把"数据如何组织成可学习对象"说清楚。输入数据被写成特征矩阵 $X\in\mathbb{R}^{n\times d}$：其中第 $i$ 行对应第 $i$ 个样本（sample），也就是该样本的特征向量 $\boldsymbol{x}_i^\top=(x_{i1},\dots,x_{id})$；第 $j$ 列对应第 $j$ 个特征（feature），表示这一属性在所有样本上的取值。输出数据写成标签向量 $\boldsymbol{y}\in\mathbb{R}^n$（或二分类时 $\boldsymbol{y}\in\{0,1\}^n$），其中 $y_i$ 与 $\boldsymbol{x}_i$ 一一配对，构成监督学习中的基本训练单位 $(\boldsymbol{x}_i,y_i)$。学习的本质是用模型 $f(\cdot;\theta)$ 把每个样本的特征向量映射为预测 $\hat{y}_i=f(\boldsymbol{x}_i;\theta)$，并通过比较 $\hat{y}_i$ 与 $y_i$ 的差异来调整参数 $\theta$。因此，图中"行=样本、列=特征、样本对输入模型产生预测"的表示方式，不仅让符号更统一，也明确了机器学习计算所依赖的最基本数据结构。

\noindent 在 $X$ 与 $y$ 的表示下，数据集由大量样本组成。样本是学习与评估的基本单位，通常对应 $X$ 的一行，也就是一个输入实例的特征向量 $\boldsymbol{x}_i$；特征对应 $X$ 的一列，表示对所有样本以同一规则测量或抽取的属性。仍以房价为例，我们可以把每套房子在预测时可获得的若干属性组织成一个数值向量。例如选取面积、卧室数、楼龄、是否地铁房四项信息，对第 $i$ 套房子，记其特征向量为
...
\text{bed}_i\\
\text{age}_i\\
\mathbb{I}(\text{near\_metro}_i)
\end{bmatrix}
\in\mathbb{R}^{4}$,
其中 $\text{area}_i$ 表示面积（平方米），$\text{bed}_i$ 表示卧室数量，$\text{age}_i$ 表示楼龄（年），$\mathbb{I}(\text{near\_metro}_i)\in\{0,1\}$ 是"是否地铁房"的指示变量。把所有 $n$ 套房子的特征向量按行堆叠，就得到输入矩阵
$X=\begin{bmatrix}
\boldsymbol{x}_1^\top\\
\boldsymbol{x}_2^\top\\
\vdots\\
...
其中 $y_i$ 是第 $i$ 套房子的成交价格。写成这种形式，并不是为了符号整齐，而是为了明确训练与部署阶段的信息边界：训练阶段模型观察配对样本 $(\boldsymbol{x}_i,y_i)$ 来学习从输入到输出的映射规律；部署阶段面对的是一套新房子对应的新向量 $\boldsymbol{x}_{\text{new}}$，模型只能使用这些在预测时可得的分量输出预测值 $\hat{y}_{\text{new}}$。将数据组织为矩阵并非为了形式美观，而是为了把对象统一成可计算的表示，从而使学习一个从 $X$ 到 $\boldsymbol{y}$ 的映射成为明确的数学问题。
...
\noindent 通过上述概念体系，学习问题获得了统一而严格的表述：用 $X$ 描述可用输入，用 $y$ 定义监督目标，以"行=样本、列=特征"的矩阵结构组织数据，并通过训练、验证、测试的隔离流程约束评估可信度，同时在必要时显式处理样本相关结构对泛化判断的影响。后续关于模型、损失函数与优化方法的讨论，均以这些基本对象与表示为基础展开。

\noindent \textbf{二\quad 模型、假设空间、参数与超参数}

\noindent 为使概念精确且便于落地，本节使用同一例子贯穿，用线性模型预测房价。设输入为特征列向量 $\boldsymbol{x} \in \mathbb{R}^{d \times 1}$，输出为真实价格 $y \in \mathbb{R}$。训练数据集记为
$\mathcal{D}=\{(\boldsymbol{x}_i,y_i)\}_{i=1}^n$。模型、假设空间、参数与超参数之间的关系如图\ref{fig-3-3-1-model-concepts}所示。
...
\noindent 图\ref{fig-3-3-1-model-concepts}把"模型—假设空间—参数—超参数"之间的关系拆解成了几个互补视角。上方首先给出模型形式（函数族模板）$\hat{y}=w^\top x+b$：它规定了候选函数必须属于某一类（这里是仿射函数），而不是直接给出某个具体函数。左侧大图将这一函数族对应到参数空间（以 $(w_1,w_2)$ 为坐标的示意）：假设空间 $\mathcal{H}=\{h_{w,b}\}$ 表示所有可能的候选函数集合，而训练得到的参数 $\theta^\ast=(w^\ast,b^\ast)$ 等价于"从集合中选出一个具体函数"；图中的曲线同时强调，优化过程是在参数空间中沿某条路径移动，学习率等训练过程超参数会影响路径形态与收敛位置（例如学习率过大可能导致次优甚至不稳定的搜索轨迹）。右上子图突出约束性超参数的作用：正则化强度 $\lambda$ 改变了"偏好什么样的解"，$\lambda$ 越大越倾向于较小权重、较平滑的解，从而在参数空间中把可接受区域"收紧"。右下子图则展示结构性超参数的含义：通过特征映射 $\phi(\cdot)$ 或多项式阶数 $M$ 做特征扩展，本质上是在改变输入表示，从而把原先较小的假设空间扩展为更高维、更强表达力的 $\mathcal{H}_\phi$；这类选择不属于一次拟合内部可自动学出的参数，而是对候选函数集合边界的外部设定。

\noindent \textbf{1）模型}

\noindent 在机器学习语境中，模型更准确地指一族参数化函数，用来近似真实但未知的映射关系。在线性回归中，我们选用如下参数化形式
$\hat{y}=f(\boldsymbol{x};\theta)=\boldsymbol{w}^\top \boldsymbol{x} + b$，
其中 $\boldsymbol{w}\in\mathbb{R}^{d \times 1}$ 为权重列向量，$b\in\mathbb{R}$ 为标量偏置，$\theta=(\boldsymbol{w},b)$ 为模型参数对。

\noindent 这一表达式的关键不在于给出某个具体预测值，而在于规定了预测函数必须属于仿射函数这一类。换言之，模型在此阶段只是定义了允许的函数形态，即线性叠加，而具体形态由参数决定。模型形式一旦确定，后续训练与评估都在这一形式的约束下展开。

\noindent 线性是相对于特征表示 $\boldsymbol{x}$ 而言的。若对原始变量做非线性变换并将其作为新特征，例如 $\log(\text{距离})$、$\text{面积}^2$、$\text{面积}\times\text{学区}$ 等，模型仍保持对特征向量 $\boldsymbol{x}$ 的线性，但在原始变量上可表现为非线性。因此，模型表达能力并非只由公式是否线性决定，还取决于特征表示的构造方式。严格地说，模型形式与特征表示共同决定了可表达的函数集合。

\noindent \textbf{2）表示与归纳偏置}

\noindent 任何学习算法都不可能在所有可能函数上有效搜索，因此必须通过模型形式与特征表示引入结构性约束。这种约束可视为归纳偏置，即在观察有限样本后，我们倾向于选择哪类函数作为解释数据的规律。

\noindent 在线性模型中，归纳偏置体现在，在给定表示 $\boldsymbol{x}$ 下，输出随特征变化呈线性叠加，模型倾向于用各特征的加权贡献解释预测。这种偏置带来的直接后果是，当真实关系接近线性或可由合适特征变换线性化时，线性模型在样本较少、噪声存在的情况下往往更稳定。当真实关系强非线性且特征表示不足时，线性模型会出现系统性偏差，表现为欠拟合。
...
$\mathcal{H}=\{h_{\boldsymbol{w},b}(\boldsymbol{x})=\boldsymbol{w}^\top \boldsymbol{x}+b \mid \boldsymbol{w}\in\mathbb{R}^{d \times 1},\, b\in\mathbb{R}\}$。
这里每一组 $(\boldsymbol{w},b)$ 都定义了一个具体假设 $h_{\boldsymbol{w},b}$，而 $\mathcal{H}$ 是所有这类仿射函数的集合。
...
\noindent 参数是由训练过程直接从数据中估计的量。在线性模型中，参数就是 $(\boldsymbol{w},b)$。训练的目标是在给定数据集 $\mathcal{D}$ 与某个训练准则下，选出最合适的参数，从而确定最终使用的具体假设 $h_{\boldsymbol{w},b}$。

\noindent 为了把参数学习说得严谨，需要明确训练准则。以最常用的平方损失为例，经验风险为
$\hat{R}(\boldsymbol{w},b)=\frac{1}{n}\sum_{i=1}^n \big(y_i-(\boldsymbol{w}^\top \boldsymbol{x}_i+b)\big)^2$。
参数学习就是求解
$(\boldsymbol{w}^\ast,b^\ast)=\arg\min_{\boldsymbol{w},b}\ \hat{R}(\boldsymbol{w},b)$。
在此框架下，$(\boldsymbol{w}^\ast,b^\ast)$ 是由数据驱动的结果，数据改变，最优参数一般也会改变。由此可以看到，参数的本质是，在既定的函数族与训练准则下，对具体函数进行实例化的自由度。

\noindent 参数还承担解释性角色。在不发生严重共线性且特征尺度可比的前提下，$\boldsymbol{w}_j$ 反映了第 $j$ 个特征对预测的边际影响方向与强度。然而在实际数据中，特征相关性、尺度差异、以及特征工程引入的交互项常使单个权重的因果解释不成立，因此需要区分预测贡献意义与因果效应意义。

\noindent \textbf{5）超参数}

\noindent 超参数是不在一次参数拟合过程中由数据直接估计得到，而是由建模者预先设定或通过外层搜索选择的量。它们通常分为两类，结构性超参数与约束及过程性超参数。线性模型提供了非常清晰的例子。

\noindent \textbf{（1）约束性超参数}
...
\noindent 综上，在线性模型这一例子里，参数 $(\boldsymbol{w},b)$ 是训练要直接估计的对象。超参数如 $\lambda$、特征扩展规则、学习率等，则决定了模型容量与训练机制。二者在职责上严格分离。参数学习回答在既定学习问题下取哪个具体函数，超参数选择回答学习问题该如何定义才更有利于泛化。

\noindent \textbf{三\quad 损失函数、目标函数与优化}

\noindent 仍以房价预测为贯穿例子。设输入为特征列向量 $\boldsymbol{x} \in \mathbb{R}^{d \times 1}$，输出为真实价格 $y \in \mathbb{R}$。训练数据为
$\mathcal{D}=\{(\boldsymbol{x}_i,y_i)\}_{i=1}^n$。
可以把它理解为 $n$ 条历史成交记录，每条记录是一套房子的信息与成交价配对样本。损失函数、目标函数与优化的关系如图\ref{fig-3-3-loss-optimization}所示。
...
\noindent 从图\ref{fig-3-3-loss-optimization}可以更直观地理解训练为何本质上是一个数值优化问题。图中的三维曲面表示目标函数 $J(\theta)$ 在参数空间中的取值，等高线表示目标值相同的区域；从高处指向低处的路径与箭头表示参数的迭代更新过程：每一次迭代都根据当前位置的局部信息（例如梯度方向）对参数做一次小幅调整，把模型从"目标值较高的区域"推进到"目标值更低的区域"。图中标注的"全局最小值"对应理想情况下的最优解，也就是使目标函数达到最低点的参数位置。实际问题中，目标函数往往并不如示意图一般光滑与单峰，可能存在局部极小、鞍点或平坦区域，使得优化过程对初始化、学习率与迭代策略更加敏感。但无论具体形态如何，训练过程始终遵循同一逻辑：损失函数决定单个样本误差如何被量化，目标函数把全体样本的损失以及正则项汇总为一个可比较、可优化的标量，而优化算法则在这一"地形图"上通过迭代移动参数，持续寻找更低的目标值区域。

\noindent \textbf{1）损失函数}
...
\noindent \textbf{2）经验风险与目标函数}

\noindent 训练的基本策略是让模型在训练集上的平均损失尽可能小。平均损失称为经验风险
$\hat{R}(\theta)=\frac{1}{n}\sum_{i=1}^n \ell\big(y_i,f(\boldsymbol{x}_i;\theta)\big)$。
...
$(\boldsymbol{w}^\ast,b^\ast)=\arg\min_{\boldsymbol{w},b}\ \hat{R}(\boldsymbol{w},b)$。
训练完成后输出的是一组参数 $(\boldsymbol{w}^\ast,b^\ast)$，也就是在训练集上使平均误差最小的那条线性预测规则。
...
$r_i = y_i-(\boldsymbol{w}^\top \boldsymbol{x}_i+b)$。
它就是第 $i$ 个样本真实值减预测值的误差，正值表示预测偏低，负值表示预测偏高。

\noindent 在平方损失下，经验风险对参数的梯度为
$\nabla_{\boldsymbol{w}} \hat{R}(\boldsymbol{w},b)=-\frac{2}{n}\sum_{i=1}^n r_i \boldsymbol{x}_i,\quad
\frac{\partial \hat{R}(\boldsymbol{w},b)}{\partial b}=-\frac{2}{n}\sum_{i=1}^n r_i$。
权重 $\boldsymbol{w}$ 的更新由残差与特征的相关性驱动，某个特征在系统性低估或高估时更相关，它对应的权重就会被相应上调或下调。偏置 $b$ 则根据整体残差的平均方向调整，总体低估就上调 $b$，总体高估就下调 $b$。

\noindent 若加入岭正则 $\lambda\|\boldsymbol{w}\|_2^2$，梯度变为
$\nabla_{\boldsymbol{w}} J(\boldsymbol{w},b)=-\frac{2}{n}\sum_{i=1}^n r_i \boldsymbol{x}_i + 2\lambda \boldsymbol{w},\quad
\frac{\partial J(\boldsymbol{w},b)}{\partial b}=-\frac{2}{n}\sum_{i=1}^n r_i$。
额外项 $2\lambda \boldsymbol{w}$ 会持续把权重往 0 拉回，从而抑制权重膨胀，提升模型稳定性。

\noindent \textbf{（2）批量、随机与小批量}
...
这一量反映的是模型对已见样本的拟合程度，但不能直接保证模型对未见样本同样可靠。训练、验证与测试的划分，正是为了解决拟合与泛化之间的天然张力。训练、验证与测试数据划分的关系如图\ref{fig-3-3-4-train-val-test}所示。
...
\noindent 从图\ref{fig-3-3-4-train-val-test}可以更清晰地理解数据划分不仅是"切成三份"，更是对模型开发流程的权限边界的约束。原始数据集首先被分为训练集、验证集与测试集三部分，其中训练集（图中蓝色）进入模型训练过程，用于迭代更新参数并驱动模型的学习；验证集（图中绿色）不参与参数拟合，而是在训练过程之外用于性能评估与超参数调整，并通过反馈机制影响下一轮训练配置与模型选择；当验证集上确定"最佳模型"后，测试集（图中橙色）只在最后使用一次，用于生成最终评估报告。图中用颜色区分了三类行为：蓝色对应学习参数，绿色对应调优与选择，橙色对应最终评估。这样的流程安排强调了一个关键原则：任何会改变模型或流程决策的选择，都应由训练集与验证集驱动，而测试集不应被纳入反复试错；否则所谓"最终评估"将不可避免地变成对测试集的隐性拟合，得到的性能结论也会系统性偏乐观。

\noindent \textbf{1）从训练误差到泛化误差}
...
\noindent \textbf{4）交叉验证}

\noindent 当数据量较小且样本波动较大时，单次划分带来的评估方差可能较大，导致模型选择结果不稳定。交叉验证通过多次划分与重复训练评估来降低这种偶然性。以 $K$ 折交叉验证为例，将数据拆为 $K$ 份，每次取一份作为验证，其余作为训练，对同一超参数 $\lambda$ 得到 $K$ 个验证误差并求平均
...
\noindent 仍以房价预测为例。设真实价格为 $y$，模型预测为 $\hat{y}=f(x;\theta)$，损失函数为 $\ell(y,\hat{y})$。本节聚焦两件事：模型为何能在未见样本上保持有效，以及模型为何会出错、出错的来源与形态是什么。泛化与误差的关系如图\ref{fig-3-3-5-generalization-error}所示。
...
\noindent 从图\ref{fig-3-3-5-generalization-error}可以把"泛化为何困难"与"误差为何产生"更集中地说明清楚。上排三幅图分别给出欠拟合、良好泛化与过拟合的典型形态：当模型过于简单时，它连训练集中的主要趋势都无法捕捉，表现为训练误差高、测试误差也高，这就是欠拟合；当模型复杂度适中时，它能够抓住训练数据的主要规律，同时对未见数据仍保持较小误差，表现为训练误差与测试误差都较低，这就是良好泛化；当模型过于复杂时，它会把训练数据里的偶然波动甚至噪声当成规律"记住"，导致训练误差被压到极低，但测试误差显著升高，这就是过拟合。下方曲线进一步抽象了这一现象：随着模型复杂度从低到高，训练误差往往单调下降，而测试（泛化）误差通常呈先降后升的"U"形，中间阴影区域对应最佳泛化区，即在偏差与方差之间取得更合理折中的区间。该图所强调的核心结论是，泛化并不等价于把训练误差压到最低，而是要在模型表达能力与稳定性之间做权衡，使测试误差尽可能处在其最低附近。
...
\noindent 第一，训练误差下降只说明模型更贴合训练集，不能单独作为模型学会规律的证据，泛化必须依赖验证或测试评估与误差类型分析来确认。第二，泛化问题本质上是有效信息量与有效自由度的匹配问题：信息不足时，高自由度模型更容易不稳定；自由度不足时，模型会表现为稳定而可预期的系统性偏差。第三，误差分析优先回答"错在什么类型"，不可约、系统性还是不稳定；把误差类型识别清楚，后续改进才会从盲目试错变成可验证的工程迭代，并最终把模型调整回图\ref{fig-3-3-5-generalization-error}所示的最佳泛化区间附近。

\subsubsection*{3.3.2\quad 监督学习}

\noindent \textbf{一\quad 监督学习要解决什么问题}

\noindent 监督学习处理的是有标准答案的学习。我们手里有一批样本，每个样本包含两部分，输入 $\boldsymbol{x}$ 和对应的正确输出 $y$。把所有训练样本写成集合就是
$\mathcal{D}=\{(\boldsymbol{x}_i,y_i)\}_{i=1}^n$,
其中 $\boldsymbol{x}_i\in\mathbb{R}^{d \times 1}$ 是第 $i$ 个样本的 $d$ 维特征列向量，$y_i\in\mathcal{Y}$ 是标签或目标值。监督学习要做的事，是从这些样本对里学到一个函数
$f(\cdot;\theta):\mathbb{R}^{d \times 1}\to \mathcal{Y}$,
使得当出现新的输入 $\boldsymbol{x}$ 时，模型给出的预测 $\hat{y}=f(\boldsymbol{x};\theta)$ 尽可能接近真实的 $y$。

\noindent $\boldsymbol{x}_i$ 像题目，$y_i$ 像标准答案。训练的目的不是把题目背下来，而是从题目与答案的对应关系中总结出规则。只要规则确实抓住了数据背后的稳定关系，那么在没见过的新题上也能做对，这就是泛化。
...
\noindent 为了让学习规则变成一个可执行的过程，需要三样东西，模型、损失、优化。模型给出如何从 $\boldsymbol{x}$ 得到预测。损失函数把预测与真实的差异量化为一个数。优化算法通过调整参数让这个数变小。常见的训练目标写成
$\min_{\theta}\; J(\theta)=\frac{1}{n}\sum_{i=1}^n L\big(y_i,f(\boldsymbol{x}_i;\theta)\big)$,
也就是让训练集上的平均损失尽量小。为了避免模型只在训练集上表现得像会了，而在新数据上不稳定，通常还会加上正则化项控制模型复杂度：
$\min_{\theta}\; \frac{1}{n}\sum_{i=1}^n L\big(y_i,f(\boldsymbol{x}_i;\theta)\big)+\lambda\,\Omega(\theta)$.
可以把它理解为，一方面尽量把题做对，另一方面不要为了做对训练题而把规则弄得过于极端。监督学习的基本流程如图\ref{fig-3-3-2-supervised-learning}所示。

\noindent \begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{figures/fig-3-3-2-supervised-learning.png}
\caption{\textbf{监督学习与逻辑回归训练闭环示意图：左侧展示监督学习的一般流程——带标签训练集（特征 $X$ 与标签 $Y$）输入监督学习算法得到模型，模型在推理阶段用于对新数据做预测；右侧以逻辑回归为例展示"模型—损失—优化"的具体化——线性组合 $z=\boldsymbol{w}^\top\boldsymbol{x}+b$ 经 Sigmoid 映射为 $(0,1)$ 概率，用于分类并形成决策边界，训练通过损失函数并借助梯度下降等优化方法调整参数。}}
\label{fig-3-3-2-supervised-learning}
\end{figure}

\noindent 图\ref{fig-3-3-2-supervised-learning}把监督学习的关键链条画成了两部分。左侧强调"数据驱动的建模流程"：训练阶段把带标签的输入数据（特征 $X$ 与标签 $Y$）交给监督学习算法，算法在某个目标函数（损失）指导下拟合出模型；推理阶段则把新样本的特征送入模型，输出预测结果。右侧进一步把这一流程落到一个具体的分类模型上：先用线性组合得到分数 $z=\boldsymbol{w}^\top\boldsymbol{x}+b$，再用 Sigmoid 把分数映射到 $(0,1)$ 的概率区间，从而可以用概率阈值或等价的决策边界完成分类；训练时通过损失函数度量预测与标签的不一致，并沿着"让损失下降"的方向（例如梯度下降）迭代更新参数。换言之，左图给出监督学习的通用训练—推理框架，右图给出其中一类经典分类器把"建模、损失、优化"三要素具体化后的计算路径。

\noindent \textbf{二\quad 经典的监督学习算法——逻辑回归}
...
$P(y=1\mid \boldsymbol{x})=p,\qquad P(y=0\mid \boldsymbol{x})=1-p$。
$\boldsymbol{w}$ 表示每个特征的重要程度。某个特征的权重为正，说明它越大越支持正类。权重为负则相反。线性部分把这些支持与反对的证据加总成一个分数 $z$，Sigmoid 再把这个分数压缩到 $0$ 到 $1$ 之间，得到可解释为概率的输出。
...
$J(\boldsymbol{w},b)=\frac{1}{n}\sum_{i=1}^n L(y_i,p_i),\quad p_i=\sigma(\boldsymbol{w}^\top \boldsymbol{x}_i+b)$。
为了抑制过拟合，常加入 L2 正则化：
$J_\lambda(\boldsymbol{w},b)=\frac{1}{n}\sum_{i=1}^n L(y_i,p_i)+\frac{\lambda}{2}\lVert \boldsymbol{w}\rVert_2^2$。
这相当于鼓励权重不要过大，使模型的规则更平滑，不至于对某些特征的细小波动反应过度。

\noindent \textbf{3）训练一步到底做了什么}

\noindent 逻辑回归的一点优势是，它的参数更新可以写得非常透明。对单个样本 $(\boldsymbol{x},y)$，令
$p=\sigma(\boldsymbol{w}^\top \boldsymbol{x}+b)$，
那么交叉熵损失对参数的梯度是
$\frac{\partial L}{\partial \boldsymbol{w}}=(p-y)\boldsymbol{x},\qquad \frac{\partial L}{\partial b}=p-y$。
如果用学习率 $\eta>0$ 做一次随机梯度下降更新，就得到
$\boldsymbol{w}\leftarrow \boldsymbol{w}-\eta(p-y)\boldsymbol{x},\qquad b\leftarrow b-\eta(p-y)$。
这组式子之所以直观，是因为核心只剩下一个量 $p-y$。它就是模型的偏差方向。当真实是 $1$ 但模型给的概率偏低时，$p-y<0$，更新会把 $\boldsymbol{w}^\top \boldsymbol{x}+b$ 往上推，从而让未来对相似样本给出更大的正类概率。当真实是 $0$ 但模型给的概率偏高时，$p-y>0$，更新会把分数往下拉，让正类概率下降。训练的迭代过程，就是不断用这种偏差去修正参数。

\noindent \textbf{4）一个可以手算的计算实例}

\noindent 取一个二维样本
$\boldsymbol{x}=\begin{pmatrix}1\\2\end{pmatrix},\qquad y=1$。
设初始参数
$\boldsymbol{w}=\begin{pmatrix}0\\0\end{pmatrix},\quad b=0,\quad \eta=0.1$。
先算分数
$z=\boldsymbol{w}^\top \boldsymbol{x}+b=0$，
于是
$p=\sigma(0)=0.5$.
这表示模型一开始没有任何偏好，只能给出 $50\%$ 的正类概率。

\noindent 接着计算 $p-y=0.5-1=-0.5$，并更新参数：
$\boldsymbol{w}\leftarrow \boldsymbol{0}-0.1(-0.5)\begin{pmatrix}1\\2\end{pmatrix}
=\begin{pmatrix}0.05\\0.10\end{pmatrix},\qquad
b\leftarrow 0-0.1(-0.5)=0.05$.
再用更新后的参数对同一个 $\boldsymbol{x}$ 计算一次：
$z'=(0.05,0.10)\cdot(1,2)+0.05=0.30$,
$p'=\sigma(0.30)=\frac{1}{1+e^{-0.30}}\approx 0.574$.
只更新一步，模型对这个正类样本的预测概率就从 $0.5$ 提升到了约 $0.574$，方向是正确的。把这种更新在大量样本上重复进行，模型会逐渐学到一组权重 $\boldsymbol{w}$ 与偏置 $b$，使得不同特征组合对应的概率输出与标签规律一致，并且在新样本上也能保持稳定表现。
...
$\mathcal{D}=\{\boldsymbol{x}_i\}_{i=1}^n,\qquad \boldsymbol{x}_i\in\mathbb{R}^{d \times 1}$，
我们希望从这些样本本身出发，提取数据的结构、规律或表示，而不是学习一个"从 $\boldsymbol{x}$ 到 $y$"的映射函数。监督学习像是"有答案的练习册"，无监督学习更像是"把一堆材料摆在面前，要求你自己归纳出它们的组织方式"。
...
\noindent \begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/fig-3-3-3-unsupervised-learning.png}
\caption{\textbf{无监督学习与 K-means 聚类示意图：左侧展示无监督学习的一般流程——只有无标签数据作为输入，算法从数据自身发现结构（如聚类、降维、关联规则）；右侧以 K-means 为例展示具体步骤——先根据当前中心分配数据点到最近的簇（Assign Data Points），再更新中心位置（Update Centroids），不断迭代直至收敛（Iterate Until Convergence），最终形成稳定的聚类结果（Final Clustering Result），图中星形标记中心点，箭头表示中心移动轨迹。}}
\label{fig-3-3-3-unsupervised-learning}
\end{figure}

\noindent 图\ref{fig-3-3-3-unsupervised-learning}用一个直观对比概括了这种"从数据自身出发"的学习方式：左侧表示只有无标签数据作为输入，右侧展示无监督方法在不依赖标签的前提下，从点云中显式地"整理出"潜在结构，其中最典型的表现就是把原本混在一起的样本按相似性分成若干簇。图中同时列出了无监督学习中最常见的三类任务：聚类（Clustering）用于发现分组结构，降维（Dimensionality Reduction）用于压缩表示并保留主要变化因素，关联规则（Association Rules）用于从离散共现中发现变量之间的依赖模式。

\noindent 与监督学习相比，无监督学习最关键的差异在于训练信号来自"数据自身"。因此，问题的定义往往更依赖建模假设：你需要明确你认为数据里应该存在什么样的结构，或你希望保留什么样的信息。换句话说，无监督学习通常不是在问"预测对不对"，而是在问"用某种结构或表示去解释数据是否合理"。在缺少外部标签对错判定的情况下，评估也更偏向"内部一致性"，例如同簇样本是否足够紧密、簇与簇之间是否足够分离，或者在降维后主要信息是否仍被保留；这些标准与图\ref{fig-3-3-3-unsupervised-learning}中"从无标签点云到结构化分组"的过程是一致的。

\noindent \textbf{二\quad 经典的无监督学习算法——K-means 聚类}

\noindent 为了把无监督学习的数学对象讲清楚，下面以 K-means 聚类作为经典算法示例。聚类的目标是把样本分成若干组，使得同组样本彼此相似、不同组样本差异更大。K-means 的思想非常直接：用 $K$ 个"中心点"代表 $K$ 个簇，让每个样本归属到最近的中心，并不断调整中心位置使得簇内更紧凑。图\ref{fig-3-3-3-unsupervised-learning}右侧以"步骤化"的方式展示了 K-means 的工作机制：先根据当前中心把数据点分配到最近的簇（Assign Data Points），再根据簇内点的位置更新中心（Update Centroids），不断迭代直到中心与分配不再明显变化（Iterate Until Convergence），得到最终稳定的聚类结果（Final Clustering Result）。图中用星形标记中心点，用箭头表示中心在迭代中的移动轨迹，这强调了 K-means 本质上是在通过反复"分配—更新"把中心推向各簇的几何中间位置。

\noindent \textbf{1）目标函数与变量含义}

\noindent 设我们希望得到 $K$ 个簇，每个簇有一个中心（质心）$\boldsymbol{\mu}_k\in\mathbb{R}^{d \times 1}$，并用指派变量 $r_{ik}\in\{0,1\}$ 表示样本 $\boldsymbol{x}_i$ 是否被分到第 $k$ 个簇，其中约束为
$\sum_{k=1}^K r_{ik}=1,\quad i=1,\dots,n.$
K-means 的优化目标是最小化簇内平方误差：
$\min_{\{r_{ik}\},\{\boldsymbol{\mu}_k\}}\; J=\sum_{i=1}^n\sum_{k=1}^K r_{ik}\,\lVert \boldsymbol{x}_i-\boldsymbol{\mu}_k\rVert_2^2.$
这个目标函数只依赖于数据点与中心点的距离，没有任何标签参与。它表达的偏好很明确：每个点都应该靠近自己所属簇的中心，簇内越紧凑越好；而这也正是图\ref{fig-3-3-3-unsupervised-learning}右侧最终形成"分散成三团"的几何原因。

\noindent \textbf{2）迭代更新规则}

\noindent K-means 用交替优化来求解该问题，重复两步直到收敛。

\noindent 第一步是指派更新。在固定中心 $\{\boldsymbol{\mu}_k\}$ 时，对每个样本选择距离最近的中心：
$r_{ik}=
\begin{cases}
1,& k=\arg\min_{j\in\{1,\dots,K\}}\lVert \boldsymbol{x}_i-\boldsymbol{\mu}_j\rVert_2^2,\\
0,& \text{otherwise}.
\end{cases}$
这一步等价于"把每个点分到最近的簇"，对应图\ref{fig-3-3-3-unsupervised-learning}中的 Step 2（分配数据点），也就是每个点都指向某一个星形中心。

\noindent 第二步是中心更新。在固定指派 $\{r_{ik}\}$ 时，每个簇的最优中心是该簇内样本的均值：
$\boldsymbol{\mu}_k=\frac{\sum_{i=1}^n r_{ik}\boldsymbol{x}_i}{\sum_{i=1}^n r_{ik}}.$
这一步等价于"把簇中心移动到簇内点的平均位置"，对应图\ref{fig-3-3-3-unsupervised-learning}中的 Step 3（更新中心点），图中的箭头形象地表达了中心点向数据更密集的位置移动。两步交替后，目标函数 $J$ 单调不增，因此会在有限步内收敛到一个局部最优解，这也对应图中的 Step 4（迭代直至收敛）与最终聚类结果。

\noindent \textbf{3）一个可手算的计算实例}

\noindent 考虑一维数据（便于手算）：
$x=\{1,2,4,8\},\qquad K=2.$
任选初始中心 $\boldsymbol{\mu}_1^{(0)}=1,\ \boldsymbol{\mu}_2^{(0)}=8$。
...
\noindent 第一轮中心更新：
$\boldsymbol{\mu}_1^{(1)}=\frac{1+2+4}{3}=\frac{7}{3}\approx 2.33,\qquad
\boldsymbol{\mu}_2^{(1)}=\frac{8}{1}=8.$
...
\noindent 指派不再变化，算法收敛。最终结果是将 $\{1,2,4\}$ 归为一组，$\{8\}$ 归为另一组。这个例子展示了无监督学习的典型特征：没有任何"正确分组"作为标签，分组完全由算法对"簇内紧凑、簇间分离"这一偏好驱动；而图\ref{fig-3-3-3-unsupervised-learning}右侧从"中心移动"到"结果分团"的过程，正是这一偏好在二维空间中的几何化呈现。

\noindent \textbf{4）代码实现}

\noindent 下面用 Python 代码实现 K-means 聚类的核心迭代过程。该代码与手算实例完全对应，其中数据为一维数组 $[1,2,4,8]$，聚类数设为 $K=2$。
...
\noindent 无监督学习经常作为监督学习的前置或辅助。一方面，无监督方法可以用于探索数据结构：例如在没有标签时先通过聚类把样本分成若干自然群体，从而为后续的标注策略、抽样策略或业务分层提供依据；这与图\ref{fig-3-3-3-unsupervised-learning}左侧"从无标签点云发现分群结构"的动机一致。另一方面，无监督学习也常用于表示学习与降维，把原始特征转化为更紧凑、更稳定的表示，使监督模型在更少的标注数据下也能更高效地学习。在实践中，一个常见的组合范式是：先用无监督方法获得表示或聚类结果，再把这些结果作为特征、先验或伪标签送入监督学习模型，以提升整体效果与训练效率。

\noindent 需要强调的是，无监督学习的输出不等价于客观真理。由于缺少标签约束，结果高度依赖于目标函数、距离度量、超参数（如 $K$）以及数据预处理方式；例如在 K-means 中，改变 $K$ 或初始化中心，都可能导致不同的局部收敛结果。因而在使用无监督学习时，更重要的是明确"你希望数据呈现什么结构"以及"这种结构在下游决策中是否可用"，并通过可视化、稳定性检验或与下游任务的联动来验证其合理性。
...
其中 $\gamma\in[0,1)$ 是折扣因子，用来控制未来奖励相对于当前奖励的重要程度。强化学习的目标可以概括为：学习一条策略，使得期望回报 $\mathbb{E}[G_0]$ 最大。强化学习的基本交互结构与 Q-learning 的核心更新元素如图\ref{fig-3-3-4-reinforcement-learning}所示。

\noindent \begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{figures/fig-3-3-4-reinforcement-learning.png}
\caption{\textbf{强化学习交互闭环与 Q-learning 要素示意图：上部为智能体—环境的交互循环（动作导致环境变化，环境返回新状态与奖励）；下部以 Q 表为例展示"状态—动作"价值的存储方式、探索与利用的取舍，以及典型的 Q-learning 更新公式（含学习率与折扣因子）。}}
\label{fig-3-3-4-reinforcement-learning}
\end{figure}

\noindent 从图\ref{fig-3-3-4-reinforcement-learning}可以看出强化学习的核心闭环。上部强调交互的因果链条：智能体根据当前状态选择动作，环境在动作作用下转移并给出奖励，新的状态与奖励再回到智能体形成下一步决策依据。下部把这种闭环落到一个可学习的数值对象上：用 $Q(s,a)$ 表示在状态 $s$ 选择动作 $a$ 的长期价值，把不同状态与动作组合的估计值存进一张表（Q 表），并通过"探索（Exploration）/利用（Exploitation）"的策略在收集经验与提升回报之间权衡；右侧更新式则表明，$Q(s,a)$ 的修正来自"当前即时奖励"与"下一状态可获得的最好价值"的结合，其中学习率控制每次修正幅度，折扣因子控制未来回报的重要性。

\noindent \textbf{二\quad 经典的强化学习算法——Q-learning}
