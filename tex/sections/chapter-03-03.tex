...
\noindent \textbf{（对应图\ref{fig-3-3-1-model-concepts}的段落替换稿）} 图\ref{fig-3-3-1-model-concepts}把"模型—假设空间—参数—超参数"之间的关系拆成了几个互补视角。上方首先给出模型形式（函数族模板）$\hat{y}=w^\top x+b$：它规定了候选函数必须属于某一类（这里是仿射函数），而不是直接给出某个具体函数。左侧大图将这一函数族对应到参数空间（以 $(w_1,w_2)$ 为坐标的示意）：假设空间 $\mathcal{H}=\{h_{w,b}\}$ 表示所有可能的候选函数集合，而训练得到的参数 $\theta^\ast=(w^\ast,b^\ast)$ 等价于"从集合中选出一个具体函数"；图中的曲线同时强调优化过程是沿着某条路径在参数空间中移动，学习率等训练过程超参数会影响路径形态与收敛位置（例如学习率过大可能导致次优甚至不稳定的搜索轨迹）。右上子图突出约束性超参数的作用：正则化强度 $\lambda$ 改变了"偏好什么样的解"，$\lambda$ 越大越倾向于较小权重、较平滑的解，从而在参数空间中把可接受区域"收紧"。右下子图则展示结构性超参数的含义：通过特征映射 $\phi(\cdot)$ 或多项式阶数 $M$ 做特征扩展，本质上是在改变输入表示，从而把原先较小的假设空间扩展为更高维、更强表达力的 $\mathcal{H}_\phi$；这类选择不属于一次拟合内部可自动学出的参数，而是对候选函数集合边界的外部设定。
...
\noindent \textbf{（对应图\ref{fig-3-3-loss-optimization}的段落替换稿）} 图\ref{fig-3-3-loss-optimization}用"目标函数地形"刻画了训练为何是数值优化问题。图中的三维曲面表示目标函数 $J(\theta)$ 在参数空间中的取值，等高线表示相同目标值的区域；从高处到低处的箭头表示训练的迭代更新过程：每一次迭代都根据当前点的局部信息（例如梯度方向）把参数向"更低的目标值"推进，从而逐步沿着下降路径接近谷底位置。图中标注的"全局最小值"对应理想情况下目标函数的最优点；实际问题中也可能存在局部极小、鞍点或平坦区域，但这一示意图强调的核心是：损失函数决定"地形如何长成"，目标函数把全体样本与正则项汇总成可优化的标量，而优化算法则是在这张地形图上通过迭代移动参数，去寻找尽可能低的目标值区域。
...
\noindent \textbf{（对应图\ref{fig-3-3-4-train-val-test}的段落替换稿）} 图\ref{fig-3-3-4-train-val-test}把数据划分与模型开发流程的权限边界画成了一个闭环。原始数据集被分为三部分：训练集（蓝色）进入"模型训练过程"，用于迭代更新参数；验证集（绿色）不参与参数拟合，而是在训练过程之外用于性能评估与超参数调整，并通过反馈箭头影响下一轮训练配置与模型选择；当验证集上确定"最佳模型"后，测试集（橙色）只在最后使用一次，用于给出最终评估报告。图中用颜色区分了三类行为：蓝色对应学习参数，绿色对应调优与选择，橙色对应最终评估。这种图示化的权限分离强调了一个原则：任何会改变模型或流程决策的选择都应由训练集与验证集驱动，测试集不应被纳入反复试错，否则"最终评估"就会变成对测试集的隐性拟合。
...
\noindent \textbf{（对应图\ref{fig-3-3-5-generalization-error}的段落替换稿）} 图\ref{fig-3-3-5-generalization-error}把泛化问题集中呈现为"模型复杂度与误差的关系"。上排三幅图分别示意欠拟合、良好泛化与过拟合：欠拟合时模型过于简单，连训练集的主要趋势都抓不住，训练误差与测试误差都偏高；良好泛化时模型复杂度适中，既能贴合训练数据的主要规律，又能在未见数据上保持较小误差；过拟合时模型过于复杂，把训练数据中的噪声也当成规律记住，训练误差极低但测试误差显著升高。下方曲线进一步抽象出这一现象：随着模型复杂度从低到高，训练误差通常单调下降，而测试（泛化）误差往往呈先降后升的"U"形，中间阴影区域对应最佳泛化区，即在偏差与方差之间取得更合理的折中。该图所强调的结论是：泛化并不等价于把训练误差压到最低，而是要在模型表达能力与稳定性之间做权衡，使测试误差处在其最低附近的区间。
